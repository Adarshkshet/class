...
def process_audio():
    try:
        ...
        if lyric_input:
            ...
            if song_title:
                return jsonify({'message': f"Playing {song_title}", 'song_title': song_title, 'lyrics_output': lyrics_output})
            else:
                return jsonify({'message': "No matching song found", 'song_title': None, 'lyrics_output': lyrics_output})
        else:
            return jsonify({'message': "No audio input detected", 'song_title': None, 'lyrics_output': lyrics_output})
    except Exception as e:
        print(f"Error processing audio: {e}")
        return jsonify({'message': "Error occurred while processing audio.", 'song_title': None, 'lyrics_output': ''})
